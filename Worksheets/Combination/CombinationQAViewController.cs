...
public class CombinationQAViewController : QuesAnsViewController {
	...
	public void setSumPanel(){
		...
		for (int i = 0; i < AnsOpGOList.Count; i++) {
			sumCounter += getQAList ().getCurrentQuesAnsPair ().ansOptionList [i] * multipleCounter [i];
		}
		if (sumCounter > 0) {
	...
	public override void answerOptionEntryAnim(GameObject ansOptionObj){
		//For entry animation
	...
	public override void submitSelected(){
